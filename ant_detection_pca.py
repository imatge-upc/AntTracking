...
from docopts.help_ant_detection_pca import parse_args
from models.foreground_mask_object_detector import ForegroundMaskObjectDetector
from models.pca_utils.pca_bbox_detector import PcaBBoxDetector
...
if __name__ == '__main__':
    ...
    bbox_model = ForegroundMaskObjectDetector(backSub_func, bbox_extractor, filter_fg_func, start_write, write_images)
    detector_model = PcaBBoxDetector(bbox_model)
    ...
    with VideoCapture(input_video) as capture:
        with open(detection_file, 'w') as out_file:
            while stop_frame <= 0 or fr <= stop_frame:
                ...
                if len(bboxes) > 0:
                    # bbox = (x1, y1, w, h)
                    MOTDet_line = lambda fr, bbox : f'{fr}, -1, {bbox[0]}, {bbox[1]}, {bbox[2]}, {bbox[3]}, 1, -1, -1, {bbox[-1]:.02f}'
