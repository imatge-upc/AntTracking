...
def obb_mot_line_generator(filename, upsample_factor=1):
    # Previous version of CVAT didn't allow 4000x3000 so we used 2000x1500, now it works without down/up-sampling (Copy data into CVAT is important to make it work due to loading)
    # However, to apply models to ease the annotation, the codec ask for lower resolution than 4000x3000 so we still downsample by 2
