...
def propose_random_crop(trk, crop_width, crop_height, width, height):
    low = np.clip(np.array([trk[2] + trk[4] - crop_width, trk[3] + trk[5] - crop_height]), 0, None)
    high = np.array([trk[2] + 1, trk[3] + 1])
...
def find_intersections(tracks, initial, final):
    column_intersect = (tracks[:, 2] < final[0]) & ((tracks[:, 2] + tracks[:, 4]) >= initial[0])
    row_intersect = (tracks[:, 3] < final[1]) & ((tracks[:, 3] + tracks[:, 5]) >= initial[1])
    intersect = column_intersect & row_intersect

    return intersect

def desired_movments(unseen_tracks, initial, final):
    delta_w = np.clip(unseen_tracks[:, 2] - initial[0], None, 0) # max 0 (move left)
    delta_w = delta_w + np.clip(unseen_tracks[:, 2] + unseen_tracks[:, 4] - final[0], 0, None) # min 0 (move right)
    delta_h = np.clip(unseen_tracks[:, 3] - initial[1], None, 0)
    delta_h = delta_h + np.clip(unseen_tracks[:, 3] + unseen_tracks[:, 5] - final[1], 0, None)
...
def undesired_movments(unseen_tracks, initial, final):
    no_delta_left = np.clip(unseen_tracks[:, 2] + unseen_tracks[:, 4] - final[0], None, 0)
    no_delta_right = np.clip(unseen_tracks[:, 2] - initial[0], 0, None)
    no_delta_up = np.clip(unseen_tracks[:, 3] + unseen_tracks[:, 5] - final[1], None, 0)
    no_delta_down = np.clip(unseen_tracks[:, 3] - initial[1], 0, None)
