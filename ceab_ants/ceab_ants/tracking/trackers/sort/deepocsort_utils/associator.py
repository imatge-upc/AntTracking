...
class DeepOCSORTAssociator():
    ...
    def __init__(self, feature_extractor, first_score_function, momentum_score_function, estimations_decoder, second_score_function=None, detections_decoder=None, th_conf=0.6, th_first_score=0.3, th_second_score=0.3, inertia_weight=0.2, use_byte=False):
    ...
    def associate(self, input_, detections, tracks):
        ...
        high_matches = self.associate_high(high_detections, feats_dist, trackers, velocities, prev_obs)
        ...
        if (len(high_matches) < len(high_detections)) and ((len(high_matches) + len(low_matches)) < len(trackers)):
            ...
            second_matches[:, 1] = unmatched_detections_idx[second_matches[:, 1]]
    ...
    def associate_high(self, high_detections, feats_dist, trackers, velocities, prev_obs):
        ...
        try:
            ...
            if (pos_associations_matrix.sum(1).max() == 1) and (pos_associations_matrix.sum(0).max() == 1):
                ...
                cost = -(score_matrix + momentum_score) + feats_dist
