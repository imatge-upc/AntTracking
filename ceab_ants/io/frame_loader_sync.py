...
# TODO: New QueuedVideoLoader so modify to make it work
...
class SyncFrameLoader():
    ...
    def __init__(self, video_list, max_buffer=5, initial_frame=0, preprocess_func=None):
        ...
        self.metadata_queues = [queue.Queue() for _ in video_list]
    ...
    def _video_loader_thread(self, video_path, video_id, initial_frame=0, preprocess_func=None):
        ...
        if preprocess_func is not None:
            preprocess_func_one = lambda x, fr : (preprocess_func(x, video_id), (video_id, fr, 1))
        ...
        with QueuedVideoLoaderContext(video_path, self.queues[video_id], self.metadata_queues[video_id], Lock(), self.stop_event, initial_frame=initial_frame, preprocess_func=preprocess_func_one):
