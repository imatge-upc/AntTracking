...
@Time: 2022/3/1 9:18
@Discription: Gaussian-smoothed interpolation (GSI from StrongSORT)
"""
...
DOCTEXT = f""" 
Usage:
  interpolate_tracks.py <trkFile> <outFile> [--interval=<i>] [--tau=<t>]
  interpolate_tracks.py -h | --help

This script removes any aditional feature on the MOT file.

Options:
  --interval=<i>        Number of frames where a track can disappear [default: 20]
  --tau=<t>             Radial basis function kernel's len_scale's parameter [default: 10]
...
if __name__ == "__main__":
    ...
    path_in     = args['<trkFile>']
    path_out    = args['<outFile>']
    interval    = float(args['--interval'])
    tau         = float(args['--tau'])

    GSInterpolation(path_in, path_out, interval, tau)
