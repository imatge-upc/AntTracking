...
class DeepOCSortAssociator():
    ...
    def associate(self, frame, detections, tracks):
        # frame is not used but other associators (like the ones in a deployed system) may need it
        ...
        high_det_apparences = high_detections[:, 5:]
        apparence_score = np.stack([trk(high_det_apparences) for trk in tracks], axis=0)

        # START OF MATCHING
        high_matches = self.associate_high(high_detections, apparence_score, trackers, velocities, previous_obs)
    ...
    def associate_high(self, high_detections, apparence_score, trackers, velocities, previous_obs):
        ...
        if pos_associations_matrix.sum(1).max() == 1 and pos_associations_matrix.sum(0).max() == 1:
            ...
            cost = -(high_iou_matrix + angle_diff_score) + apparence_score
