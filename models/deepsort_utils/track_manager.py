...
class Track():
    ...
    def predict(self):
        ...
        return np.concatenate((prediction, np.full((1, 1), self.age)), axis=1) # 1, 5+4x4+1
    
    def update(self, observation):
        output = self.estimator.update(observation)
        if observation is not None:
            self.apparence_scorer.update(observation)
    ...
    def __call__(self, query):
        return self.apparence_scorer(query)

class TrackManager():

    def __init__(self, estiamtor_cls, apparence_scorer_cls, track_cls=None, max_age=1, min_hits=3, max_last_update=1, return_pred=False):
